	...
	public void runAlgorithm(String input, String output, int minUtility) throws IOException {
		...
		for (Integer item : mapItemToTWU.keySet()) {
			...
			if (mapItemToTWU.get(item) >= minUtility) {
				...
				UBP ubp = new UBP(item, pCount);
				mapItemToUBP.put(item, ubp);
		...
		Collections.sort(listOfUBPs, new Comparator<UBP>() {
			public int compare(UBP o1, UBP o2) {
				// compare the TWU of the items
				return compareItems(o1.utilityList.item, o2.utilityList.item);
			}
		});

		// SECOND DATABASE PASS TO CONSTRUCT THE UTILITY LISTS
		// OF 1-ITEMSETS HAVING TWU >= minutil (promising items)
		int numberofPatitions = 0;
		int numberOfTid = 1;
		try {
...
//					int remain = remainingUtility; // FOR OPTIMIZATION
					UBP UBPOfItem = mapItemToUBP.get(pair.item);
					Long Tid = UBPOfItem.partitions[numberofPatitions].TidSet;
					Long newLong = ChangeTidValue(Tid, numberOfTid);
					UBPOfItem.partitions[numberofPatitions].support++;
					UBPOfItem.partitions[numberofPatitions].Suru += remainingUtility;
					UBPOfItem.partitions[numberofPatitions].TidSet = newLong;
					...
					UBPOfItem.utilityList.addElement(element);
		...
		for (UBP ubp : listOfUBPs) {
			UtilityList ul = ubp.utilityList;
	...
	private Long ChangeTidValue(Long Tid, int position) {
		return modifyBit(Tid, (63 - position));
	}

	public Long modifyBit(Long n, int p) {
		long mask = 1l << p;
		return (n & ~mask) | ((1l << p) & mask);
	}
	 ...
	 * @throws IOException
	 */
	private void fhm(int[] prefix, int prefixLength, UBP pUBP, List<UBP> UBPs, int minUtility)
			throws IOException {

		// For each extension X of prefix P
		for (int i = 0; i < UBPs.size(); i++) {
			UBP X = UBPs.get(i);
			candidateCount++;
			// If pX is a high utility itemset.
			// we save the itemset: pX
			if (X.utilityList.sumIutils >= minUtility) {
				// save to file
				writeOut(prefix, prefixLength, X.utilityList.item, X.utilityList.sumIutils);
			}

			// If the sum of the remaining utilities for pX
			// is higher than minUtility, we explore extensions of pX.
			// (this is the pruning condition)
			if (X.utilityList.sumIutils + X.utilityList.sumRutils >= minUtility) {
				// This list will contain the utility lists of pX extensions.
				List<UBP> exUBPs = new ArrayList<UBP>();
				// For each extension of p appearing
				// after X according to the ascending order
				for (int j = i + 1; j < UBPs.size(); j++) {
					UBP Y = UBPs.get(j);
					// ======================== NEW OPTIMIZATION USED IN FHM
					Map<Integer, Long> mapTWUF = mapFMAP.get(X.utilityList.item);
					if (mapTWUF != null) {
						Long twuF = mapTWUF.get(Y.utilityList.item);
					...
					UBP temp = construct(pUL, X, Y, minUtility);
					if (temp != null) {
						exUBPs.add(temp);
					}
				}
				// We create new prefix pX
				itemsetBuffer[prefixLength] = X.utilityList.item;
				// We make a recursive call to discover all itemsets with the prefix pXY
				fhm(itemsetBuffer, prefixLength + 1, X, UBPs, minUtility);
			}
		}
		MemoryLogger.getInstance().checkMemory();
	}
	...
	public void printStats() throws IOException {
		System.out.println("=============  UBP ALGORITHM =============");
